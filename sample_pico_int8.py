...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Sample from FULL int8 quantized picoGPT for on-device deployment"
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="out-pico-shakespeare-char",
    ...
    parser.add_argument(
        "--max_new_tokens", type=int, default=500, help="Max new tokens to generate"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="float32",
        help="Data type for non-quantized operations",
    )
    parser.add_argument(
        "--compile", action="store_true", help="Compile model for faster inference"
    )
    parser.add_argument(
        "--show_stats", action="store_true", help="Show detailed model statistics"
    )
...
def load_int8_model(out_dir):
    ...
    if os.path.exists(quantized_model_path):
        ...
        print(
            f"Pre-quantized model not found, creating FULL int8 model from checkpoint..."
        )
...
def create_quantized_model_on_demand(out_dir):
    ...
    checkpoint_path = os.path.join(
        out_dir, "ckpt_l3_h4_e192_bs32_bl128_lr3e-3_iter8k_baseline.pt"
    )
